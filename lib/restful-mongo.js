...
RestfulMongo.prototype.configure=function(app, options){
    ...
    if(options.methods) {
        ...
        methods.indexOf('post') >= 0 && self._setRunScript(app)
...
RestfulMongo.prototype._setRunScript=_setRunScript
...
function _setRunScript(app) {
    ...
    app.post('/run-script/?', function(req, res) {
        ...
        var codes = req.body ? req.body : null;
        ...
        if (codes!=null){
            ...
            for(var i=0; i<codes.length; i++){
                var code = codes[i];
                var code_to_run = code.code_to_run;
                var src = code.src;
                var new_src = eval(code_to_run);
                results.push({"new_src": new_src, "src": src});
            }
        }
...
function _setGetCount(app) {
    ...
    app.get('/api-count/:db/:collection/:id?', function(req, res) {
        ...
        var fields = {}
        var test = ['limit', 'sort', 'skip', 'exfields',  'hint', 'explain', 'snapshot', 'timeout', '$exist'];
...
function _setGet(app) {
    ...
    app.get('/api/:db/:collection/:id?', function(req, res) {
        ...
        var options = req.params.options || {};
        var fields = {}
        var test = ['limit', 'sort', 'fields', 'exfields', 'skip', 'hint', 'explain', 'snapshot', 'timeout', '$exist'];
...
function _setPut(app) {
    ...
    app.put('/api/:db/:collection/:id?', function(req, res) {
        ...
        try{
            ...
            if (req.body['$set'] && req.body['$set'].updated_time==null){
            ...
            //}
            //else{
                self.Dao.connectionPool.getDb(dbName, self.Dao.options.db.username,  self.Dao.options.db.password, function(err, db) {
                    if(!err) {
...
function _setDel(app) {
    ...
    app.del('/api/:db/:collection/:id', function(req, res) {
        ...
        self.Dao.connectionPool.getDb(dbName,  self.Dao.options.db.username,  self.Dao.options.db.password,  function(err, db) {
            if(!err) {
                db.collection(req.params.collection, function(err, collection) {
                    collection.remove(spec_id, function(err, docs) {
