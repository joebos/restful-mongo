...
Dao.prototype.query = function(dbName, collectionName, query, fields, options, cb) {
    ...
    var conn_pool_string = T.connectionPool.getPooledConnections();

    console.log("query > POOL: " + conn_pool_string);
    ...
    T.connectionPool.getDb(dbName, this.options.db.username,  this.options.db.password, function(err, db) {
        if(!err) {
            db.collection(collectionName, function(err, collection) {
                ...
                var debug_info = {db: collection.db.databaseName, col: collection.collectionName};
                console.log("query > collection: " + JSON.stringify(debug_info, null, 2));
...
Dao.prototype.get = function(dbName, collectionName, query, fields, options, cb) {
    ...
    console.log("get > POOL: " + conn_pool_string);
    ...
    T.connectionPool.getDb(dbName, this.options.db.username,  this.options.db.password, function(err, db) {
        if(!err) {
            db.collection(collectionName, function(err, collection) {
                ...
                var debug_info = {db: collection.db.databaseName, col: collection.collectionName};
                console.log("get > collection: " + JSON.stringify(debug_info, null, 2));
...
Dao.prototype.save = function(dbName, collectionName, body, cb) {
    ...
    console.log("save > POOL: " + conn_pool_string);
...
*/              var docs=body;
                ...
                var debug_info = {db: collection.db.databaseName, col: collection.collectionName};
                console.log("save > collection: " + JSON.stringify(debug_info, null, 2));
...
//                dup_query = dup_key + ': { $in: [ ' + dup_values.join(',') + ' ]}';
                ...
                collection.update(hide_query, hide_update, {upsert:false, multi:true, safe:true, w:1, j: true}, function(err, doc) {
                    collection.update(dup_query, dup_update, {upsert:false, multi:true, safe:true, w:1, j: true}, function(err, doc) {
                        collection.insert(docs, {writeConcern:{w:1, j: true}}, function(err, docs) {
                            cb(err, docs)
                        });
                    });
                });
